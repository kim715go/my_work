...
newValidOnes['diffR'] -= 100

plots = list(newValidOnes.columns[:7])
plots.remove('bRD')

columnNames = ['인문사회계', '이공계', '예술계', '행정지원', '연구시설', '강의지원', '편의시설', '학술지원']
subNames = ['EUI(원단위 사용량, (kWh/㎡·year)', '기저부하율(시간단위)(%)', '난방기간 에너지 변동비(%)', '냉방기간 에너지 변동비(%)', '냉방 대비 난방 에너지 부하율(%)', '전년대비 사용량 증감율(%)']
fileNames = ['원단위', '기저율', '난방률', '냉방률', '냉방대비율', '증감률']

for index, usage in enumerate(plots) : 

    test = newValidOnes[[usage, 'usageCode']]
    ...
    for i in range(8) : 
        ...
        temporar = test.query('usageCode=='+str(i))
        temporar = temporar[usage]
    ...
    plt.suptitle(subNames[index], fontsize=16)
    fig.tight_layout(rect=[0, 0.03, 1, 0.95])
    #plt.show()
    #fig.tight_layout()
    fig.savefig(fileNames[index]+'.png')
    plt.close(fig)