# "전력량 초기 보정 및 그래프 확인 프로그램 v.1.0.5 (2019/7/19)"
...
# v 1.0.5 수정사항 : 파일 저장시 열려 있는 파일이어서 permissionError가 나는 경우 대응, 폴더에 동일한 명칭의 파일이 있을 때 확인하도록 처리 (2019/7/19)
...
class Kernel(uI.Ui_MainWindow):
    ...
    def openFile(self, path):
        try :
            ...
            return 'missingFile'
        if table.shape[1] != 24 :
            QtWidgets.QMessageBox.warning(self.centralwidget, "올바르지 않은 파일 형식", "테이블의 형태가 활용 가능하지 않습니다.\n" + path, QtWidgets.QMessageBox.Ok)
    ...
    def parseInitialTable(self, path, table, resNotShowing = False):
        temp = table.stack()
        temp.name="전력량"
        temp.index = pd.date_range(table.index[0], freq='h', periods=len(temp))
    ...
    def showTable(self, tableData):
        ...
        if not isinstance(tableData, str) :
    ...
    def saveGraphFile(self):
        if not self.currentGraphSaveFolderPath :
        ...
        if self.onlyCurrentRadioButton.isChecked() :
            ...
            for path in self.filePathList :
                ...
                if path in self.parsedFileTables :
                    ...
                    if isinstance(tempTable, pd.DataFrame) :
    ...
    def modifyFileName(self, bldName, extension, existingFileList):
        # if xxx.xxx exists in existingFileList -> make file name xxx (2).xxx, xxx (3).xxx, etc...
        tryNum = 2
        fileName = "".join([bldName, ".", extension])

        while True :
            if fileName in existingFileList :
                fileName = "".join([bldName, " (", str(tryNum), ").", extension])
                tryNum += 1
            else :
                break
    ...
    def saveDataFile(self):
        if not self.currentDataSaveFolderPath :
            QtWidgets.QMessageBox.warning(self.centralwidget, "파일 경로 오류", "데이터 파일 저장 경로를 먼저 설정해 주세요", QtWidgets.QMessageBox.Ok)
            return

        finishMessage = QtWidgets.QMessageBox(self.centralwidget)
        finishMessage.setWindowTitle("저장 확인")
        finishMessage.setText("파일이 저장되지 않았습니다")
        ...
        existingFilesInSaveFolder = listdir(self.currentDataSaveFolderPath)

        singleOverwriteMessage = QtWidgets.QMessageBox(self.centralwidget)
        singleOverwriteMessage.setWindowTitle("덮어쓰기 오류")
        singleOverwriteMessage.setIcon(QtWidgets.QMessageBox.Warning)
        singleOverwriteMessage.setStandardButtons(QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No|QtWidgets.QMessageBox.Save)
        singleOverwriteDiffNameButton = singleOverwriteMessage.button(QtWidgets.QMessageBox.Save)
        singleOverwriteDiffNameButton.setText("이름을 바꾸어 저장")
        singleOverwriteMessage.setText("저장 경로 폴더에 동일한 이름의 파일이 존재합니다.\n덮어쓸까요?")

        multipleOverwriteMessage = QtWidgets.QMessageBox(self.centralwidget)
        multipleOverwriteMessage.setWindowTitle("덮어쓰기 오류")
        multipleOverwriteMessage.setIcon(QtWidgets.QMessageBox.Warning)
        multipleOverwriteMessage.setStandardButtons(QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No|QtWidgets.QMessageBox.Save|QtWidgets.QMessageBox.Cancel)
        multipleOverwriteDiffNameButton = multipleOverwriteMessage.button(QtWidgets.QMessageBox.Save)
        multipleOverwriteDiffNameButton.setText("이름을 바꾸어 저장")
        applyAllCheckBox = QtWidgets.QCheckBox("모든 충돌에 동일하게 적용")
        multipleOverwriteMessage.setCheckBox(applyAllCheckBox)
        
        multipleOverwriteMessageText = "저장 경로 폴더에 아래와 동일한 이름의 파일이 존재합니다.\n덮어쓸까요?"

        if self.onlyCurrentRadioButton.isChecked() :
            if self.currentPath in self.parsedFileTables:
                bldNameWithExtension = self.currentPath.split('/')[-1]
                bldName = bldNameWithExtension.split(".")[0]
                if bldNameWithExtension in existingFilesInSaveFolder :
                    res = singleOverwriteMessage.exec_()
                    if res == QtWidgets.QMessageBox.Yes :
                        saved = self.pivotAndSave(self.parsedFileTables[self.currentPath]['table'], "".join([self.currentDataSaveFolderPath, '/', bldName, '.csv']), encoding)
                        if saved :
                            finishMessage.setText("파일이 저장되었습니다")
                        else :
                            finishMessage.setText("파일 쓰기 권한이 없습니다. 같은 이름의 파일이 열려 있는지 확인하세요.\n파일이 저장되지 않았습니다.")
                    elif res == QtWidgets.QMessageBox.No :
                        pass
                    else : #QtWidgets.QMessageBox.Save
                        newFileName = self.modifyFileName(bldName, 'csv', existingFilesInSaveFolder)
                        saved = self.pivotAndSave(self.parsedFileTables[self.currentPath]['table'], "".join([self.currentDataSaveFolderPath, "/", newFileName]), encoding)
                        if saved :
                            finishMessage.setText("파일이 저장되었습니다")
                        else:
                            finishMessage.setText("파일 쓰기 권한이 없습니다. 같은 이름의 파일이 열려 있는지 확인하세요.\n파일이 저장되지 않았습니다.")
            ...
            collisionBehavior = {'applyAll':False, 'howToHandle':None}

            for path in self.filePathList:
                bldNameWithExtension = path.split('/')[-1]
                bldName = bldNameWithExtension.split('.')[0]
                self.statusbar.showMessage(" ".join([bldName, '파일 저장 중...']))

                if bldNameWithExtension in existingFilesInSaveFolder:
                    if not collisionBehavior['applyAll'] :
                        res = multipleOverwriteMessage.exec_()
                        if applyAllCheckBox.isChecked() :

                            collisionBehavior['applyAll'] = True
                            collisionBehavior['howToHandle'] = res
                    else :
                        res = collisionBehavior['howToHandle']
                    if res == QtWidgets.QMessageBox.Yes :
                        fileName = bldNameWithExtension
                    elif res == QtWidgets.QMessageBox.No :
                        dataNotSaved.append(path + " (충돌 회피)")
                    elif res == QtWidgets.QMessageBox.Save :
                        fileName = self.modifyFileName(bldName, "csv", existingFilesInSaveFolder)
                    else : #QtWidgets.QMessageBox.Cancel
                        return
                ...
                if path in self.parsedFileTables:
                    ...
                    if isinstance(tempTable, pd.DataFrame) :
                        ...
                        if tempTable == 'missingFile' :
                            reason = " (파일 경로 오류)"
                        elif tempTable == "wrongStructure":
                            reason = " (파일 형식 오류)"
                        else :
                            reason = " (원인 불명)"
                        dataNotSaved.append(path + reason)
                        continue

                if (startDate_str in table.index) and (endDate_str in table.index):
                    res = self.pivotAndSave(table, "".join([self.currentDataSaveFolderPath, '/', bldName, '.csv']), encoding)
                    if res :
                        pass
                    else :
                        dataNotSaved.append(path + " (쓰기 권한 없음)")
                else:
                    dataNotSaved.append(path + " (기간 오류)")
